...
def build_manifest_from_pipeline_state(state: "PipelineState") -> Optional[ProvenanceManifest]:
    """Build a ProvenanceManifest from a completed PipelineState's real
    results, or ``None`` if the run wasn't actually completed successfully -
    never fabricates a manifest for a failed/partial run.
...
def persist_provenance_if_completed(
    ...
    """Build and atomically persist a ProvenanceManifest for a completed
    PipelineState, returning the written path - or ``None`` if the run
    wasn't actually completed/successful, or if the write itself failed
    (a filesystem error here never retroactively fails an already-completed
    pipeline run; it's best-effort supplementary provenance, not a required
