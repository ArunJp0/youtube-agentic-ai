...
## Publishing through the main pipeline is explicit opt-in, never automatic on a PASS

Simply running the content-generation pipeline to a genuine Compliance PASS must never, by itself, cause a real YouTube API call - a caller has to deliberately ask for publishing. `PublishingIntent` (`src/models/youtube_upload.py`) defaults to `mode="disabled"`, and `build_pipeline_graph`/`run_pipeline` only ever construct a `YouTubeUploadAgent`/reach the `publishing` node when a caller explicitly passes both a non-disabled `PublishingIntent` and a `YouTubeClient`. A non-disabled intent with no client is a configuration error raised immediately at graph-build time, not a silently-ignored request or a deep node failure.

## The publishing integration reuses YouTubeUploadAgent unchanged; no second uploader

`publishing_node` in `src/workflows/pipeline_graph.py` calls the exact same `YouTubeUploadAgent.publish()` the standalone `youtube_upload_demo.py` CLI already calls - same OAuth/`YouTubeClient` abstraction, same deterministic Compliance-PASS gate, same `PublishingRecordStore` idempotency guard, same centralized scheduling validation. The only new code is how the input `DiscoveredRunArtifacts` gets built: the pipeline constructs it directly from the run's own real in-memory `PipelineState` fields (final video path, `MetadataResult`, thumbnail path, `ComplianceResult`), never via the standalone CLI's `discover_latest_run_artifacts()` disk-globbing (which exists specifically for a caller with no in-memory state to read from - the pipeline always has one).

## Compliance PASS gates entry to the publishing node at the routing layer, not just inside the agent

`route_after_compliance` only ever returns `"publishing"` when `state.compliance_result.publish_decision == "PASS"` AND publishing was explicitly enabled - REVIEW and BLOCK can never reach the node at all, regardless of whether publishing was requested. This is a second, independent enforcement of the same rule `YouTubeUploadAgent.publish()` itself already checks (belt-and-suspenders, not redundant guessing): even a hypothetical future bug inside the agent's own gate check would still not matter, since REVIEW/BLOCK states never reach a point where the agent is even called.

## A real end-to-end publishing validation reused an existing real PASS run instead of re-running the pipeline, once a genuine LangGraph constraint was discovered

Real validation of the publishing integration needed a genuine Compliance PASS candidate, and re-running the full expensive real pipeline (Gemini/Pexels/Edge TTS/Whisper/FFmpeg) purely to reach one was both wasteful and explicitly out of scope for that validation. The intended alternative - resuming a compiled LangGraph pipeline directly at its `publishing` node against an already-completed run's real artifacts - turned out to be impossible without either a full re-run or adding checkpoint/resume infrastructure: LangGraph's `StateGraph` has a single fixed entry point (`research`), and pre-populating a state's upstream fields does not skip re-execution of the entry node. Rather than building that infrastructure under real-validation time pressure (an architecture change outside this milestone's scope), the real validation instead invoked the exact same production call `publishing_node` makes (`YouTubeUploadAgent.publish()` with a `DiscoveredRunArtifacts` constructed identically to the node's own source) directly against the real, already-produced artifacts for run `why-is-the-ocean-salty-a06dc312` - proving the integration's actual runtime logic against real OAuth/API while the `route_after_compliance` routing transition itself remains covered by the 13 mocked integration tests. Checkpoint/resume support is recorded as optional future infrastructure (useful for exactly this kind of "resume publishing against an older completed run" scenario, and for crash recovery), not a requirement this milestone was blocked on.

## Topic Planner follows the existing provider-abstraction pattern - TopicSourceProvider, not a bespoke discovery mechanism

Just as every other external capability in this project (search, LLM, voice, media, transcription, music catalog, YouTube upload) sits behind a small abstract interface with real and mock implementations, topic-candidate discovery follows the identical shape: `TopicSourceProvider` (`src/tools/topic_source_provider.py`) declares one method (`discover_candidates`), with `MockTopicSourceProvider`, `YouTubeTopicSourceProvider`, and `CurrentNewsTopicSourceProvider` as interchangeable implementations. `TopicPlannerAgent` depends only on the interface, so a future additional source (Google Trends, Reddit, etc. - explicitly deferred, not built here) can be added without touching the agent at all.

## Current/trending news is sourced via free Google News RSS - no paid API, no new dependency

Consistent with "no paid APIs during initial development," `CurrentNewsTopicSourceProvider` uses Google's public, keyless RSS feed rather than a paid news API (e.g. NewsAPI.org's paid tiers) or a heavier aggregation service. RSS parsing uses Python's stdlib `xml.etree.ElementTree` - no new third-party dependency was added for this milestone. Region (`gl`/`ceid`) and language (`hl`) are native RSS query parameters Google already supports; a category or free-text sub-national region (which Google News has no native geo-targeting for) becomes a search query term instead - the same code path handles both, with no per-region special-casing.

## Composite source merging keeps TopicPlannerAgent's own constructor untouched

Rather than changing `TopicPlannerAgent.__init__` to accept a list of sources (which would have broken every existing call site/test from the prior milestone), a new `CompositeTopicSourceProvider` wraps multiple `TopicSourceProvider`s and implements the same single-provider interface - `TopicPlannerAgent` still takes exactly one `topic_source_provider`, which may itself be a composite. Each component source is called independently; one failing is recorded and skipped (never raised) as long as at least one succeeds, so a real current-news outage never crashes planning when YouTube (or vice versa) is still available - only if every component fails does the composite raise, preserving the existing "source_unavailable" contract.

## Cross-source confidence is a corroboration count, never a hardcoded outlet trust list

STEP 5's requirement ("avoid selecting a story solely because one unknown source published it") was deliberately implemented WITHOUT naming any specific news outlet as more or less trustworthy - that would require curating and maintaining a trust list, and would itself be a form of hardcoded bias. Instead, `distinct_source_count` counts how many independently-named publishers reported substantially the same story (via the existing near-duplicate normalization), and a small deterministic step function (1 source → 0.4, 2 → 0.7, 3+ → 1.0) turns that count into a bounded confidence score. A single-source story is never zeroed out - it's a real, if lower-confidence, signal, not disqualifying.

## Market and category preference are soft scoring weights, never hard filters

A candidate outside the configured target market(s) or preferred categories is never excluded from selection - it scores a mild, deliberately-not-punishing 0.3 (vs. 1.0 for a match, 0.5 when the signal isn't configured at all), so a sufficiently strong story on every other signal can still win. A candidate whose market is "global" (or unknown) always scores maximally relevant regardless of what markets are configured, including when the configuration is exactly `["global"]` itself (a real bug found and fixed during this milestone's own real validation prep - see the Known limitations note below). This directly implements the explicit requirement that "a globally important story should still be selectable even when regional signals exist," and equally for category.

## Region/language are plain configuration strings; the planner has no embedded geography

`TOPIC_TARGET_MARKETS`/`TOPIC_LANGUAGE`/`TOPIC_CATEGORIES` flow through `TopicPlannerAgent` and `TopicSourceProvider` implementations as opaque strings - neither ever branches on a specific region/language value in business logic. This was verified directly: the identical code path was exercised end-to-end against `"GB"`, `"IN"`, `"Tamil Nadu"` (a free-text sub-national region with no native Google News geo-targeting parameter, handled generically via the search-query fallback), and `"global"`, with no region-specific branch anywhere in `TopicPlannerAgent`/`CurrentNewsTopicSourceProvider`.

## Sensitive-story detection is a flag-only heuristic, never a second Compliance Agent

Topic Planner is explicitly not a fact-checker or a duplicate of the Copyright/Compliance Agent - `src/services/topic_sensitivity.py`'s keyword heuristic only sets `TopicCandidate.is_sensitive`/`sensitivity_reasons` (propagated to the top-level `TopicSelectionResult`) for a downstream Research/Compliance stage to apply stricter treatment later. It never suppresses, rejects, rewrites, or auto-fabricates a decision about the story - consistent with "do not automatically fabricate or suppress stories using LLM opinion" (this mechanism is not even LLM-based; it's a plain deterministic keyword match, an even stronger guarantee against opinion-driven suppression).

## A real Google News RSS redirect had to be explicitly followed - httpx's default silently broke real usage

Real standalone validation (`TOPIC_MODE=trending`) failed 100% of the time against the live Google News RSS endpoint despite passing every mocked test, because both the world-feed and search URLs respond with an HTTP 302 redirect to the actual feed content, `httpx.AsyncClient()` defaults to `follow_redirects=False`, and httpx's own `raise_for_status()` treats an un-followed redirect as an `HTTPStatusError`. The fix is one line (`follow_redirects=True` on the provider's internally-constructed client) - a real-network behavior that no amount of mocked testing (which injects a client directly, bypassing real redirect handling entirely) could have caught, underscoring why one real standalone validation was required before considering this milestone genuinely done.

## AutonomousRunController is coordination-only - every capability it uses is injected, never constructed by it

`AutonomousRunController` (`src/orchestration/autonomous_run_controller.py`) takes a `TopicPlannerAgent` and a `PipelineRunner` callable (`(topic, publishing_intent) -> PipelineState`) as constructor arguments - it never imports a provider, never builds a `TopicSourceProvider`/`LLMProvider`/`YouTubeClient` itself, and contains no Research/Script/Video/Compliance/YouTube logic of its own. Real provider wiring lives entirely in a separate module (`src/orchestration/autonomous_factory.py`), which builds both the real `TopicPlannerAgent` and a closure around the existing, unchanged `run_pipeline(...)` using the exact same `src.config.providers` factory functions `pipeline_demo.py`/`topic_planner_demo.py` already use. This keeps the controller's own logic testable with pure in-memory stubs (no mocked HTTP/Gemini/FFmpeg needed to test lock/idempotency/status-mapping behavior) and guarantees the orchestration layer can never silently duplicate pipeline logic - it can only call the same single `run_pipeline` entry point every other real caller already calls.

## The run lock is a durable file, never an in-memory flag - and needs no new dependency

STEP 6 of the Autonomous Orchestration milestone explicitly required a lock that is not "only an in-memory boolean." `RunLock` (`src/services/run_lock.py`) uses `os.O_CREAT | os.O_EXCL` to atomically create a marker file containing the holder's PID and UTC acquisition timestamp - the same "atomic file write" idiom already used by every `*_store.py` in this project, rather than adding a third-party dependency (no `filelock` in `pyproject.toml`). A lock older than the configured `AUTONOMOUS_LOCK_TIMEOUT` is treated as stale and automatically removed by the next `acquire()` call (covering a crashed holder that never released it), and `release()` is always called from a `finally` block in the controller, so a lock is never left held after any exception, including one the controller itself didn't anticipate.

## The autonomous scheduler is a minimal hand-rolled cron matcher, never a new scheduling framework

Consistent with this project's explicit "no unnecessary frameworks" rule (already applied to BGM catalog choice, RSS parsing, etc.), `AutonomousScheduler` does not depend on Celery, Redis, Kafka, APScheduler, or `croniter` - `src/orchestration/cron_schedule.py` implements just enough of standard 5-field cron syntax (`*`, `*/n`, ranges, lists) with the stdlib only, and the scheduler itself is a plain sequential `async def run_forever()` loop that calls `AutonomousRunController.run_once("scheduled", scheduled_time=...)` exactly once per occurrence it computes. `tzdata` was added as the one new dependency, not because of scheduling logic but because stdlib `zoneinfo` has no bundled IANA tz database on Windows - a no-op on Linux/macOS, which use the OS-provided database instead.

## Autonomous idempotency is a single "was this exact scheduled occurrence already terminal" check, not distributed idempotency

Per the milestone's explicit "do not overengineer distributed idempotency" instruction, `AutonomousRunController` only checks `AutonomousRunStore.find_by_scheduled_time(scheduled_time)` before starting work for a given occurrence - if a record for that exact occurrence already reached a terminal (non-`"started"`) status, the existing record is returned unchanged rather than starting another run. A `"started"`-but-never-finished record (e.g. the process crashed mid-run) is deliberately NOT treated as terminal, so that occurrence gets a fresh attempt rather than being permanently stuck. Upload-level duplicate protection continues to rely entirely on the pre-existing `PublishingRecordStore` idempotency guard inside `YouTubeUploadAgent.publish()` (unchanged) - the autonomous layer adds occurrence-level idempotency on top, it does not reimplement upload-level idempotency.

## A durable run-outcome record must always propagate the underlying result's own diagnostic warnings

Real validation surfaced a genuine gap: `AutonomousRunController` was building `AutonomousRunRecord` from `PipelineState.publishing_result` but only copying `.status`/`.video_id`/`.video_url` - not `.warnings`, which is the only place a partial-failure explanation (e.g. "video uploaded successfully, but setting the thumbnail failed: <reason>") actually exists once `UploadResult` itself goes out of scope. Since `AutonomousRunRecord` is the sole durable record of an autonomous run once the process exits, silently dropping warnings meant a `thumbnail_failed`-style outcome was persisted with zero explanation. Fixed by propagating `result.warnings` into `record.warnings` in both the `"published"` and `"publishing_failed"` branches of `_apply_pipeline_outcome`. The general principle - any layer that summarizes a richer typed result into a smaller durable record must carry forward that result's own diagnostic/warning fields, not just its headline status - applies to this pattern generally, not only to this one field.

## The OAuth client credential identifies the application, not a channel - the same client secret is reused across channel switches

Switching the real YouTube uploader from the personal/test channel to a newly created dedicated client channel ("What's Behind It") required no code change and no new Google Cloud OAuth client: `secrets/youtube_client_secret.json` (a Desktop-app OAuth client) identifies this application to Google, while `secrets/youtube_token.json` identifies which authorized Google account the application is currently acting as. Re-running the existing, unmodified `get_credentials(..., interactive=True)` flow with no valid token present naturally re-prompts for account selection and issues a fresh token for whichever account signs in - the old token was preserved as a rename-only backup (`secrets/youtube_token.personal-backup.json`), never deleted, and revoking/replacing a token never affects the underlying YouTube channel itself (confirmed and recorded during this migration's audit). This establishes the safe, repeatable procedure for any future channel switch: back up the token file, re-run the existing `--authenticate` flow, verify the new `channels.list(mine=True)` identity before doing anything else - never touch the client secret.

## A newly created YouTube channel needs phone verification before custom thumbnails work - an account state, not a pipeline defect

The first real upload to the new "What's Behind It" channel succeeded, but `videos.thumbnails.set` failed with HTTP 403 ("The authenticated user doesn't have permissions to upload and set custom video thumbnails") - YouTube's standard restriction on a channel that hasn't completed phone verification in YouTube Studio yet. This is recorded as an operational fact about onboarding any *new* channel with this project (not something the code can detect or work around), distinct from the pre-existing "OAuth project likely unverified, may force private uploads" note already on record for the original personal channel - custom thumbnails specifically require channel-level (not just OAuth-project-level) verification. The existing `thumbnail_failed` partial-success handling (video preserved, thumbnail failure captured as a warning, never a full upload failure) already handled this correctly without any code change; once verification was completed, retrying only `set_thumbnail` against the same already-uploaded video (no re-upload) succeeded.

## Autonomous topic selection defaulted to a fixed mock catalog by configuration, not by design intent

`TOPIC_MODE`/`TOPIC_NEWS_SOURCES_ENABLED`/`TOPIC_PLANNER_SOURCE` all shipped with conservative defaults (`evergreen`/`false`/`mock`) so that adding the Topic Planner never silently changed any existing deployment's behavior. That conservatism had a side effect nobody had explicitly set: `.env` never opted into anything else, so every autonomous run kept drawing from the same fixed 5-item mock catalog indefinitely. The scoring/ranking logic itself was audited and found not to be the cause (a well-corroborated fresh candidate does outrank an equally-judged evergreen one). The fix was `.env`-only - `TOPIC_MODE=mixed`, `TOPIC_NEWS_SOURCES_ENABLED=true`, `TOPIC_PLANNER_SOURCE=youtube` - confirming the original design (mixed real evergreen + real trending, degrading gracefully without a `YOUTUBE_API_KEY`) already supported exactly what was needed; it simply had never been turned on.

## Script duration is a centralized word budget, not a per-caller hardcoded section count

Before this phase, `ScriptAgent.max_sections` defaulted to a bare literal (`5`), and every prompt asked for a fixed sentence count regardless of any target video length. `src/services/script_duration.py` centralizes both the profile-name-to-minutes mapping (`short`/`standard`/`long`) and the word-budget derivation formula in one place, so no other module ever hardcodes a duration number. The key design choice: `WordBudget.words_per_section(actual_count)` redistributes the *entire* body-word budget across however many sections research actually produced, rather than pre-computing a fixed per-section word count up front - a shorter real research result reaches roughly the same target duration through more substantial per-section narration (explicitly instructed in the prompt to develop with real depth, never pad/repeat), never by inventing extra sections. This was deliberately NOT extended to `ResearchAgent`'s own "5-7 key points" prompt in this phase - real testing showed depth-compensation alone was sufficient to reach the `standard` profile's 5-8 minute target even with as few as 6 real key points, so touching Research (a separate, working, already-validated agent) wasn't necessary.

## AI Video Generation is a new, separate provider abstraction - not an extension of MediaProvider

`MediaProvider` (Pexels) is a search-then-download interface built around an existing asset library; AI video generation is fundamentally prompt-in/clip-out with no search step, no pre-existing candidate list, and provider-specific request semantics (aspect ratio, duration, model, cost). Rather than bolting generation onto `MediaProvider`'s contract, `AIVideoProvider` (`src/tools/ai_video_provider.py`) is a distinct abstraction with its own typed request/result models (`src/models/ai_video.py`), reusing only the downstream `MediaAsset` model so a successful AI-generated clip is indistinguishable in shape from a Pexels asset to Visual QC/Video Assembly/provenance/compliance - no new result type propagates past the point of acquisition.

## The AI-video/Pexels routing decision is a standalone, independently-testable function, not inline branching inside VisualMediaService

`acquire_visual_asset_with_ai_fallback` (`src/services/ai_video_visual_acquisition.py`) implements "try AI generation (bounded retries) → Pexels fallback (if enabled) → structured failure" as a pure function taking the existing Pexels selection chain as an injected callable, rather than writing that branching logic directly inside `VisualMediaService._acquire_slot_asset`. This let the routing decision itself be fully unit-tested (retry bounds, fallback-enabled/disabled, exact-failure-message content) in isolation before ever touching the real, heavily-tested `VisualMediaService`, and kept the eventual integration (`_acquire_visual_slot`, a thin wrapper) to a single new guarded call site - when `ai_video_provider` is `None` (the default), it behaves byte-for-byte identically to calling `_acquire_slot_asset` directly.

## LocalAIVideoProvider is explicitly labeled a demo/local adapter, never allowed to look like a real generation provider

`LocalAIVideoProvider.generate()` always reports `model="manual-local-demo-clip"` (a literal constant, never a real model name) and appends an explicit warning whenever it reuses an existing clip for an unmatched section/slot - so a result can never be mistaken, downstream or in a persisted record, for genuine autonomous API generation. This directly enforces the project's "never claim manually-generated clips were produced autonomously by an API" requirement structurally, not just by convention.

## One real PixVerse demo clip was assembled through a standalone demo script, not the integrated VisualMediaService path

`src/ai_video_demo.py` hand-builds a `VisualResult`/`ScriptResult` and calls `VoiceService`/`VideoAssemblyService`/`CaptionService`/`AudioMixingService` directly, rather than exercising the real `run_pipeline`/`VisualMediaService.ai_video_provider` integration end to end. Two reasons: (1) with exactly one 5-second source clip for a whole video, duration-aware multi-slot planning (`calculate_slot_count`) would request far more distinct visual slots than one clip (even with reuse) can meaningfully serve without degrading into obvious repetition - a scenario the normal production planner was never designed for; (2) keeping this entirely in a separate, demo-specific script guarantees zero risk to the real, validated production pipeline while still reusing every actual production component (Voice/Video Assembly/Captions/BGM services, `FFmpegVideoAssembler`) completely unchanged. The real `VisualMediaService` integration seam remains real, tested, and intended for the eventual paid-provider path, where normal multi-slot generation (not a single hand-placed clip) is the expected mode of use.

## Demo-only topic selection may use an isolated history context without touching real dedup, but never bypasses it

Preparing real script/research content for the AI-video demo needed a topic with genuinely strong research coverage; the best candidate had already been selected in real production history (blocked by `TopicPlannerAgent`'s own duplicate-prevention, which checks both provenance-produced and topic-plan-selected history). Rather than disabling or special-casing that dedup logic, the demo run constructed its `TopicPlannerAgent` with `topic_plan_store`/`provenance_output_dir` pointed at empty, isolated temp directories - the real dedup mechanism still runs unmodified, just scoped to a context where "has this exact video already been produced or planned in THIS context" is correctly "no" (true for a demo that produces no real, published video). This establishes the pattern for any future non-production topic-selection exercise: scope the history, never bypass the check.

## Research provider selection is topic-source-CHARACTERISTIC-aware, never topic-text/content-aware

A real demo validation found a current/trending candidate falling back to evergreen because `ResearchAgent` always searched Wikipedia regardless of where the topic came from. The fix deliberately does not inspect the topic string's own words/content to decide where to search (a keyword/content heuristic would be a topic-specific hack, and the task explicitly ruled that out) - instead, `ResearchAgent.research(topic, topic_source=...)` accepts the Topic Planner's own real classification of the winning candidate (`TopicCandidate.source`, e.g. `"current_news"`, surfaced through to `TopicSelectionResult.selected_topic_source`) and routes purely on that. `topic_source=None` (or any value other than `"current_news"`) preserves the exact original single-provider (Wikipedia) behavior, byte-for-byte, for every existing call site.

## CurrentNewsSearchProvider reuses the existing Google News RSS infrastructure rather than duplicating it

`CurrentNewsTopicSourceProvider` (topic discovery) already had working, real-validated Google News RSS fetch/parse/redirect-handling logic. Rather than a second independent implementation for Research's own needs, the shared fetch (`_fetch_rss_xml`) and parse (`_parse_rss_items`/`_parse_pub_date`) logic was factored out of that provider and reused unchanged by the new `CurrentNewsSearchProvider(SearchProvider)` in the same module. The one deliberate difference: `CurrentNewsSearchProvider` surfaces the RSS `<description>` snippet (HTML-cleaned) as research context, since Research legitimately needs more than a bare headline to work with - topic discovery's own "never store full article bodies" constraint is untouched and still enforced by its own dedicated test.

## A deterministic, length-only minimum-substance gate replaces silent thin-content synthesis, scoped to the multi-provider path only

Before this fix, `ResearchAgent` would hand whatever search results it got - however thin or tangential - straight to the LLM to make the best of. For a current-news topic where neither `CurrentNewsSearchProvider` nor a Wikipedia fallback produces enough real material, silently proceeding would risk a script built on largely-invented content. `_has_sufficient_substance()` is a deliberately simple, provider-agnostic combined-snippet-word-count check (`DEFAULT_MIN_CONTEXT_WORDS = 40`) - never a content/relevance judgment, so it can never become a topic-specific heuristic - and when it fails on the multi-provider (current-news-aware) path, `ResearchAgent` now raises an explicit `ResearchAgentError` instead of proceeding. This gate is intentionally never applied to the default single-provider path, preserving that path's original, already-validated tolerant behavior exactly.

## Provenance fields on ResearchResult/TopicSelectionResult are additive, not a breaking schema change

`ResearchResult.source_provider`/`source_published_at` and `TopicSelectionResult.selected_topic_source`/`selected_topic_source_url`/`selected_topic_published_at` are all new optional fields with safe defaults (`None`/`[]`). Every existing caller - `ScriptAgent`, the pipeline workflow, every pre-existing test - continues to work unchanged; only code that explicitly reads the new fields (the new routing/reporting logic itself) is affected. This follows the same additive-trailing-optional-field pattern already established for `script_word_budget`/`ai_video_provider`/etc.

## The key-point preamble parsing fix is narrow and targeted, not a parser rewrite

`ResearchAgent._parse_bullet_points` had a real, reproducible bug: a leading, unmarked, colon-terminated LLM meta-commentary line (e.g. "Based on the provided source material, here are 5 key points:") was counted as a fabricated first key point. The fix tracks whether a line is the first non-blank line seen (not raw list index, so leading blank lines in the response are handled correctly) and drops it only when it has no bullet/number marker of its own AND ends with `:` - a genuine first bullet point that happens to end in `:` (e.g. "Key finding one: temperatures rose significantly.") is never touched, since it already carries its own marker. No other parsing behavior changed.

## A test-isolation gap, not a production bug, caused a publishing-mode default test to fail

`TestBuildPublishingIntent::test_defaults_to_disabled` read the real local `.env`'s `AUTONOMOUS_PUBLISHING_MODE=private` (set deliberately for controlled real-validation runs) because it constructed `Settings()` with no environment isolation, while sibling tests in the same file already isolated themselves by popping the relevant env var first. `build_publishing_intent` itself was already correct - the fix was isolating the one test that lacked the same guard, not a code change.

## Research validation must be a provider-independent contract, not a per-provider patch

A real controlled validation exposed that relevance/substance checking had only ever been wired into `CurrentNewsSearchProvider`'s own code path (`_run_search_provider`'s `if provider is not self.current_news_search_provider: return results` branch) - Wikipedia, used both as the evergreen default and as the current-news fallback, returned completely raw, unvalidated results, and a genuine current-news topic's Wikipedia fallback once produced a long, real, but entirely wrong-topic script with no error raised. Patching Wikipedia specifically would have repeated the same mistake for the next provider added later. The fix instead moves validation into `ResearchAgent` itself (`_search_and_validate`/`_classify_and_dedupe`), applied identically to every `SearchProvider` regardless of identity - current-news, Wikipedia, or any future provider inherits the same safety behavior automatically through the orchestration layer, rather than needing to remember to implement its own checks.

## DIRECT/SUPPORTING/UNRELATED classification replaces a binary relevant/not-relevant judgment

A pure relevant/not-relevant filter can't express "this is a legitimate comparison or piece of background context, but not itself evidence about the topic" - collapsing that nuance either rejects genuinely useful supporting material or, worse, lets it count toward sufficiency as if it were direct evidence. `ResearchRelevanceFilter`'s existing single batched LLM call was extended (not duplicated with a second call) to return a three-way classification instead of a boolean. Sufficiency is checked against DIRECT content only - SUPPORTING material is preserved and may still reach `combined_context` (clearly labeled, with explicit prompt guidance not to let it dominate), but can never by itself satisfy the substance gate. An UNRELATED classification still requires the same defensible-relationship bar as before; the prompt explicitly forbids inventing a loose connection just to avoid it.

## `simplify_query` ranks tokens by informativeness instead of cutting at a fixed position

The original first-N-words truncation assumed the topic's subject tends to appear early in the sentence - false for a common real headline shape ("Here's what X thinks happened in [the actual subject]"), where truncation kept the generic opening clause and silently dropped the subject entirely, observed directly in a real validation run. The fix ranks each token (capitalized/proper-noun-like first, other significant non-stopword terms second, generic filler last, via a stable sort so ties keep original order) and keeps the top-ranked terms, re-emitted in their original sentence position - never scrambled, never LLM-generated, still exactly one bounded retry.

## Research failure must always be explicit, including the previously-tolerant zero-result and single-provider paths

Before this pass, a genuine zero-results case returned a "successful" empty `ResearchResult` (a non-empty `summary` of "No sources found..."), and the single-provider (evergreen) path had no substance gate at all - both could reach `ScriptAgent`, risking a hallucinated or near-empty script from nothing. Both branches were removed in favor of one uniform rule: `ResearchAgentError` is raised whenever DIRECT content is insufficient, on every path, no exceptions. This is a deliberate behavior change (not a bug fix to preserve tolerance) - failing safely is preferable to publishing a wrong-topic or content-free video.

## `research_focused_claim` intentionally stays outside the universal quality contract

Compliance Remediation's bounded single-claim research refresh (`ResearchAgent.research_focused_claim`) calls its configured search provider directly and was audited as a genuine, narrower validation gap - but it produces one `ResearchFact` correcting an already-vetted script section, not a new `ResearchResult`/standalone script content, and it already has its own safety net (the LLM must respond `NO_EVIDENCE` rather than fabricate when the source material doesn't address the claim). Routing it through the full DIRECT/SUPPORTING/UNRELATED contract was left out of this pass to avoid touching Compliance Remediation's own behavior - it remains a documented, separately-scoped limitation, not silently folded into "normal" research.

## Current-news freshness is a window, not a same-day-only requirement

Real validation showed a selected current-news topic's own supporting search results span several days to weeks (the triggering story is fresh, but Google News also surfaces genuinely relevant older background coverage of the same ongoing situation). Requiring every result to be same-day would have made the substance gate fail routinely for otherwise perfectly usable research. Roughly the recent 3-7 day window is treated as "current," while older reliable sources remain acceptable as background/context - mirroring the role Wikipedia already plays on the evergreen path - rather than encoding a strict same-day cutoff anywhere in the routing or substance-gate logic.

## Topic-level content continuity is a new orchestration layer above ResearchAgent, not a change to ResearchAgent's own provider fallback

`ResearchAgent` already had its own bounded provider fallback (current-news → Wikipedia) for a SINGLE topic. Exhausting the ranked topic CANDIDATES a Topic Planner run produced was a separate, higher-level problem, and folding it into `ResearchAgent` would have mixed "which sources to try for this one topic" with "which topic to try next" - two different concerns with different failure semantics (content-insufficient vs. genuinely-out-of-candidates). `TopicContinuityOrchestrator` (`src/orchestration/topic_continuity_orchestrator.py`) sits one level above both `TopicPlannerAgent` and `ResearchAgent`, calling each unchanged, so neither had its own internal contract altered by this work.

## The multi-tier fallback is opt-in via one constructor param, so the original single-shot controller behavior is byte-for-byte preserved by default

`AutonomousRunController` gained one new optional `topic_continuity` param defaulting to `None`. When unset, `_execute()` takes the exact original code path (one `plan_topic()` call, immediately handed to the pipeline) with zero behavior change - verified by every pre-existing controller test passing unmodified. This avoided threading five new dependencies (research agent, evergreen planner, reserve store, per-tier bounds) into the controller's own constructor; they live inside `TopicContinuityOrchestrator` instead, which the controller only ever calls through one method.

## Infrastructure failure needs its own exception type, because Gemini's own retry/fallback has already run out by the time ResearchAgent sees the error

A candidate can fail Research for two structurally different reasons: the topic genuinely lacks sufficient material (try the next candidate), or Gemini/search/network/auth is down (trying a different candidate hits the identical problem and wastes bounded attempts for nothing). `ResearchInfrastructureError` (a subclass of `ResearchAgentError`) is raised only when every configured search provider for that call raised outright (never even returned content to evaluate), or when LLM synthesis itself raised - by the time an LLM-processing exception reaches `ResearchAgent`, Gemini's own documented retry/backoff/fallback-model policy has already exhausted itself, so this is a genuine "Gemini is down right now" signal, not something a different topic would fix. Being a subclass means every existing `except ResearchAgentError` call site (e.g. `research_node`) keeps catching it identically - this is purely additive specificity for the new orchestrator to act on.

## The qualified topic reserve stores identity and provenance only, never cached research content

TIER 3 needed to satisfy "never assume old time-sensitive facts remain current" without building a separate staleness-window algorithm. The simplest mechanism that is trivially correct: `QualifiedTopicReserveEntry` never stores a `ResearchResult` at all - only the topic string, its source classification, and qualification provenance/timestamp. A reserve topic is always re-researched fresh, through the exact same `ResearchAgent.research()` call every other tier uses, at the moment it is actually consumed; if that fresh check fails, the entry is consumed (never retried indefinitely) rather than assumed still valid. The reserve is also deliberately evergreen-only (populated only from TIER 2's own discovery), sidestepping the need for any current-news staleness-window logic entirely.

## The reserve is seeded opportunistically from TIER 2's own leftover candidates, not by a separate background job

Requiring a dedicated replenishment process would be new standing infrastructure for a bounded, low-frequency need. Instead, whenever TIER 2 runs a dedicated evergreen-only Topic Planner call, a small bounded number (`TOPIC_CONTINUITY_RESERVE_SEED_ATTEMPTS`, default 2) of its own remaining, untried ranked candidates are opportunistically research-validated and persisted for a FUTURE run - at most a few extra Research calls, only when TIER 2 already ran anyway, never a scheduled job of its own. No video is ever pre-generated merely to populate this reserve.

## The bounded corroboration retry must not be skipped just because a topic is already short

Real validation exposed that `ResearchAgent._search_and_validate`'s retry compared the simplified query to the original topic and skipped the retry entirely when they were equal (`simplify_query` returns the topic unchanged once it's already short/terse) - meaning an already-short, thin-initial-result topic got zero retry attempts at all. The fix keeps the retry bounded to exactly one attempt (never a second retry tier) but removes the skip: when there's nothing to simplify, the retry re-queries the SAME topic string, requesting more candidates than the initial search (`DEFAULT_RETRY_RESULT_MULTIPLIER = 3`, centralized). This widens evidence ACQUISITION only - `DEFAULT_MIN_CONTEXT_WORDS`, title-only detection, near-duplicate collapse, and DIRECT/SUPPORTING/UNRELATED classification are all untouched, so a thin headline still can never be accepted as sufficient evidence on its own.

## A single Visual QC-rejected asset must not be a whole-video failure - only a section with zero safe coverage may be

Two real autonomous runs both hard-stopped the entire pipeline before Video Assembly over exactly one Visual QC-rejected stock asset, even though every other section's media was fine. The root cause was `visual_qc_node`'s hard-stop condition itself: `qc_result.rejected_count > 0` treated any single asset's unresolved rejection as a whole-video failure, with no notion of "drop this one optional clip and keep going." The fix introduces a PASS/REPLACEABLE/CRITICAL disposition model (`src/models/visual_qc.py`): CRITICAL is now a SECTION-level judgment (zero safe usable visual coverage after every recovery strategy is exhausted), never a single-asset-level one - `visual_qc_node` now checks `qc_result.disposition == "critical"` instead. This preserves Visual QC's own semantic judgment/thresholds/misleading-detection completely unchanged; only the POLICY for what to do with a still-bad asset after bounded replacement changed.

## A rejected asset is dropped, not kept, when bounded replacement is exhausted - section coverage is recomputed, not assumed

Before this fix, `VisualQCService._resolve_slot`'s replacement loop kept whatever asset the LAST attempt produced, even if it was still classified weak/misleading - the "best-available" asset was, in the worst case, a known-bad one that made it all the way into the final video's candidate list (and, previously, into `rejected_count`, triggering the whole-pipeline hard stop above). The fix drops such an asset instead - replaces it with a `success=False` placeholder (preserving its original provenance fields for diagnostics) so `VideoAssemblyService`'s EXISTING usable-asset filter (`_map_usable_assets_by_section`) and per-section duration redistribution (`slot_duration = duration / len(assets)`) automatically handle a section with fewer clips than originally planned - no VideoAssemblyService change was needed at all. "Sufficient coverage" is recomputed immediately afterward from what actually remains, never assumed from the original slot count.

## The neutral fallback is a locally-generated background/title-card, never reused stock content from elsewhere in the video

When a section would otherwise end up with zero usable visual coverage, the two most obvious options - reusing an already-QC-approved asset from a DIFFERENT section, or lowering the QC bar to let a weak asset through - were both rejected. Reusing a different section's clip risks visually asserting something that section's own narration doesn't - the same "unrelated visual = misleading" problem Visual QC exists to catch, just self-inflicted. Lowering the bar directly contradicts the whole point of this milestone. Instead, `NeutralVisualGenerator`/`FFmpegNeutralVisualGenerator` (`src/tools/neutral_visual_generator.py`) produces a plain color background (FFmpeg's `lavfi color` source) with, at most, an already-known-safe plain-text label (the section's own heading) - it never depicts photographic content and therefore can never itself assert an unsupported factual claim, by construction rather than by another round of semantic judgment. Kept as its own small interface (not a new `VideoAssembler` abstract method) so this optional, rarely-needed capability never has to be implemented by every existing `VideoAssembler` test double across the suite.

## A QC-driven replacement must report failure, never silently reuse an unrelated or excluded asset

Auditing why 11 replacement downloads in a real run still left one asset unresolved found a second, compounding bug: `VisualMediaService._acquire_slot_asset`'s desperate final fallback (when no new, plan-approved candidate exists anywhere in the query chain) reused ANY already-downloaded asset in the whole video - including, in the true last-resort branch, one already in the caller's own `exclude_ids`. A "replacement" that is really an unrelated clip from a different section stands almost no chance of passing QC for THIS section, so bounded replacement attempts were sometimes silently wasted on a doomed reuse rather than a genuinely new candidate. The fix adds `allow_reuse_fallback` (default `True`, preserving normal initial slot-filling's existing, validated reuse-when-Pexels-is-exhausted behavior exactly); `acquire_replacement_asset` (the QC-driven entry point) now passes `False`, so a replacement attempt either finds a genuinely new candidate or reports clean failure - never a disguised non-replacement. The same fallback was also fixed to always respect `exclude_ids` even for normal acquisition, closing a latent gap that could have handed back an explicitly excluded asset in a sufficiently degenerate case.

## Recognizing a locally-generated neutral fallback in Compliance's provenance check is accurate, not permissive

Wiring the neutral fallback through a real pipeline run surfaced that Compliance's `check_visual_provenance` correctly, conservatively blocked on the new `provider="neutral_fallback"` string, since `KNOWN_VISUAL_PROVIDERS` only listed `"pexels"`/`"mock"`. This was the RIGHT default reaction to an unrecognized provider - but a procedurally generated color-and-text background genuinely carries no licensing/attribution risk at all, unlike real stock media. Adding `"neutral_fallback"` to `KNOWN_VISUAL_PROVIDERS` teaches Compliance an accurate fact about a new, itself-safe asset origin - the same way `"mock"` is already recognized for test fixtures - rather than weakening what the check verifies for `"pexels"` assets (which still require a real `source_url`, unchanged).

## TIER 2 evergreen discovery must not have a single real source

The very next real controlled autonomous run after Visual QC Resilience ended `topic_candidates_exhausted` even though Topic Continuity's architecture existed end to end. TIER 1 had legitimately exhausted all 5 ranked current-news candidates (genuine Research rejection, not a threshold problem), but TIER 2 was structurally unavailable: its only real discovery source was `YouTubeTopicSourceProvider`, and `YOUTUBE_API_KEY` was not configured in production. A tier that exists architecturally but has exactly one real, currently-unusable source is not a genuine fallback - it is a disguised single point of failure. The fix does not touch Research (still the same unweakened contract) or invent a new resilience mechanism inside the continuity orchestrator - it fixes TIER 2 at the source level, by giving it a second real, independent discovery path.

## WikipediaTopicSourceProvider reuses the existing Wikipedia search infrastructure rather than inventing a new discovery mechanism

The requirement was a generic, provider-independent, no-new-paid-dependency evergreen source. Wikipedia's public MediaWiki search API was already trusted and already wrapped by `WikipediaSearchProvider` for Research - `WikipediaTopicSourceProvider` (`src/tools/wikipedia_topic_source_provider.py`) takes a `SearchProvider` as a dependency (a real `WikipediaSearchProvider()` by default) rather than writing a second HTTP client, and turns real search results for a small, centralized, CONFIGURABLE list of generic category seed queries (science, technology, history, geography/nature, "how/why" explainers) into `TopicCandidate`s. The seeds are deliberately category-shaped search terms, never a hardcoded list of final video topics - the actual discovered titles are always whatever Wikipedia genuinely returns. One seed's failure is caught and skipped (mirroring `CompositeTopicSourceProvider`'s own component-level resilience, applied here one level down); only if every seed fails does the provider raise `TopicSourceProviderError`.

## YouTube and Wikipedia are composed via the existing CompositeTopicSourceProvider, not a new fallback mechanism

`CompositeTopicSourceProvider` already implemented exactly the required semantics before this milestone: try each component, skip one that raises, merge whatever the successful ones return, and only raise if every component fails. Rather than building new "try YouTube, then try Wikipedia" logic inside `TopicContinuityOrchestrator` or `TopicPlannerAgent`, `get_resilient_evergreen_topic_source_provider` (`src/config/providers.py`) simply composes `YouTubeTopicSourceProvider` (still tried first, still fully capable of succeeding alone when configured) with the new `WikipediaTopicSourceProvider` via this existing class - scoped to TIER 2's own evergreen planner only (`build_evergreen_topic_planner_agent` in `autonomous_factory.py`), never touching `get_topic_source_provider`/`get_topic_planner_source` (the PRIMARY planner's unrelated, unchanged evergreen path) or Research/Visual QC/any other pipeline stage.

## An inner client timeout is not sufficient on its own - external calls need an explicit outer bound too

A real controlled autonomous run hung for over 100 minutes inside Visual Media acquisition - Python alive, near-zero CPU, no FFmpeg process, an established-but-silent outbound connection - even though both `GeminiLLMProvider` (via `VisualContextPlanner`) and `PexelsMediaProvider` already configured their own `httpx` client-level timeouts. Relying solely on an inner library timeout proved insufficient in practice. The fix is defense in depth, not a replacement: every network call in the Visual Media path now ALSO has an explicit outer `asyncio.wait_for` ceiling wrapped around the actual call site, set comfortably above the inner timeout's own worst case so it only ever engages as a genuine last-resort safety net, never cutting off a legitimate (if slow) attempt early. This pattern - inner timeout for the normal case, outer `wait_for` as an unconditional hard backstop - is the one to reach for anywhere else in this codebase a blocking or async network call could, in principle, wait longer than its own configured timeout suggests.

## VisualContextPlanner's synchronous Gemini call is bounded via asyncio.to_thread + wait_for, not by making it async

`VisualContextPlanner.plan_visuals()` wraps a plain synchronous `LLMProvider.generate_text()` call and was being invoked directly inside async `VisualMediaService.build_plan()` - meaning a stuck call blocked the entire asyncio event loop, not just its own coroutine. Rewriting `VisualContextPlanner`/`LLMProvider` to be async would have touched every LLM call site in the project (Research, Script, Compliance, Metadata, Thumbnail) for a fix scoped to one Visual Media reliability gap. Instead, `build_plan()` now runs the existing synchronous call via `asyncio.to_thread(...)`, wrapped in `asyncio.wait_for(..., timeout=360s)` - this bounds the wait AND keeps the event loop responsive, without changing `LLMProvider`'s interface or any other caller. A timeout is treated exactly like any other planning failure: it degrades to the same deterministic fallback plan `plan_visuals()` already produces internally, never raised past `build_plan()`.

## Pexels timeouts degrade through existing bounded infrastructure, not a new retry mechanism

Pexels failures already had two independent, existing bounded-recovery layers before this milestone: `VisualMediaService._acquire_slot_asset`'s query-chain fallback (a `MediaProviderError` from one query tier moves to the next - specific, then neutral, then last-resort) for search, and `_download_new_asset`'s typed-failure conversion (any exception becomes a `success=False` `MediaAsset`, never propagated) for download. A timeout was made to raise the exact same `MediaProviderError` both paths already handle, rather than adding a third, parallel retry mechanism - satisfying "use existing bounded retry infrastructure where already available" literally, and avoiding any risk of nested/unbounded retry interaction between a new mechanism and the existing ones.

## Fixing the neutral-fallback test-isolation leak required threading a directory parameter through, not just fixing the tests

The leaked `b"NEUTRAL"` placeholder files were reachable because `VisualQCService`'s `neutral_fallback_output_dir` had no path from `build_pipeline_graph`/`run_pipeline`'s own `media_output_dir` parameter at all - it silently used its own class-level default (the real `output/media`) regardless of what directory the rest of a given pipeline run was configured to use. Patching only the test helper would have left the same trap for the next test (or real caller) that injects a `neutral_visual_generator` without knowing to also override this previously-invisible parameter. The fix adds `neutral_fallback_output_dir` as an explicit parameter on both `build_pipeline_graph` and `run_pipeline` (defaulting to the exact same production value, so real runs are unaffected), and the test suite now always passes its own tmp_path-scoped directory for it.

## The External-Boundary Production Readiness Audit was scoped to bounding, not redesigning

The Visual Media Network-Boundary Reliability fix resolved a real 101-minute hang, but its root cause was never conclusively narrowed to only Gemini planning or only Pexels - leaving open whether the same "inner client timeout doesn't reliably fire in this environment" failure mode existed at any other production-reachable boundary. Rather than assuming so and adding timeouts everywhere defensively, every remaining boundary (Research providers, Edge TTS, remaining Gemini calls, Visual QC's vision calls, FFmpeg/ffprobe, faster-whisper, thumbnail dependencies, YouTube OAuth/API/upload/thumbnail) was individually read and classified as either a genuine external/network/subprocess call needing a bound, or legitimate local compute/an already-reliable OS-level mechanism needing none - consistent with the project's standing "don't add arbitrary timeouts everywhere" constraint. Where a genuine gap was found, the fix was always the smallest one that preserved every existing retry/fallback and added a bound at the actual call site, never a broader refactor of the pipeline or its policies.

## GeminiLLMProvider's outer bound uses a manually-managed ThreadPoolExecutor, not a `with` block

`generate_text` is a plain synchronous method (per the existing `LLMProvider` interface contract) using synchronous `httpx.post()`, so it can't be bounded with `asyncio.wait_for` the way the project's other (already-async) providers were. The natural-looking `with concurrent.futures.ThreadPoolExecutor(...) as executor:` was deliberately avoided: `__exit__` calls `shutdown(wait=True)`, which blocks until the background thread finishes even after a timeout was already raised - defeating the entire point of an outer bound. The executor is instead constructed directly and `shutdown(wait=False)` is called explicitly in both the timeout and success branches, so a stalled call can never make the outer bound itself hang.

## Edge TTS gets an outer bound because it exposes no timeout of its own at all

Unlike Wikipedia/Google News/YouTube topic discovery (which already had an inner `httpx` client timeout that just needed a defense-in-depth outer backstop), `edge-tts` talks to Microsoft's TTS service over a websocket with no configurable or documented timeout exposed to callers - meaning a stalled connection had literally no bound before this audit, not merely an unreliable one. The fix wraps the entire streamed synthesis call in `asyncio.wait_for(timeout=120s)`, sized generously above realistic synthesis time (typically much faster than real-time even for a long script), and reuses the provider's existing partial-file cleanup-on-failure logic for both the timeout and generic-failure paths - no new cleanup mechanism was added.

## GoogleYouTubeClient needed an explicitly constructed httplib2 transport, not just a wrapped call site

Every other fix in this audit added an outer bound around an existing call, but `googleapiclient.discovery.build(credentials=...)` doesn't expose a call-site-level timeout hook at all - it internally builds its own `httplib2.Http()` transport with `timeout=None` (confirmed via `inspect.signature`), so the gap lived in how the client's transport was constructed, not in how it was called. The fix constructs that transport explicitly - `google_auth_httplib2.AuthorizedHttp(credentials, http=httplib2.Http(timeout=600s))` passed to `build(http=...)` instead of `build(credentials=...)` - using `google-auth-httplib2`, which was already an explicit (not merely transitive) project dependency. 600s (not the ~20-120s used elsewhere in this audit) matches `FFmpegVideoAssembler`'s own precedent for large media operations, since `insert_video` uploads the entire video file in one request (`MediaFileUpload(..., chunksize=-1)`) and a real large file over a slow connection can legitimately take several minutes - the bound exists to catch a truly stalled connection, not to cap a merely slow one.

## A stalled YouTube transport is retried through the existing upload-retry loop, not a second retry mechanism

`insert_video`'s chunked upload loop already retried a bounded number of times (with backoff) on a retryable 5xx `HttpError`. A transport-level timeout/connection drop mid-upload is exactly as transient as a 5xx response, so `TimeoutError`/`ConnectionError`/`OSError` are caught in the same loop and consume the same `attempt`/`_MAX_UPLOAD_RETRIES`/`_RETRY_BACKOFF_SECONDS` budget, raising `YouTubeClientError` only once genuinely exhausted - avoiding a second, independent retry policy that could interact unpredictably with the first.

## Thread-based hang tests use a short simulated delay; coroutine-based ones can safely use a long one

`asyncio.sleep(3600)` inside a plain coroutine is safe to use in a hang test even with a millisecond-scale timeout override, because `asyncio.wait_for`'s timeout cleanly cancels the awaited coroutine. A `time.sleep(...)` (or any blocking-thread wait) run via `asyncio.to_thread`/`ThreadPoolExecutor` is different: Python threads can't be forcibly killed, and `concurrent.futures.thread`'s own `atexit` handler joins every such thread before the interpreter can exit - so a 3600s blocking sleep would hang the entire test process at exit, not just time out the one test. Every thread-based hang test in this project (`GeminiLLMProvider`) therefore uses a short delay (2s) that safely exceeds its own drastically-reduced outer-timeout override (e.g. 0.05s) without meaningfully delaying process exit if the worker thread outlives the test.
