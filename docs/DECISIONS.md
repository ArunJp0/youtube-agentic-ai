...
- The Gemini fallback model was changed (`gemini-3.6-flash` → `gemini-3.1-flash-lite`) based on a real health check, but a real full 8-stage end-to-end run with the new configuration (all real providers together, through BGM) has not yet been performed - that is the next planned milestone. Gemini free-tier model availability/quota can change over time, so the health check should be re-run periodically rather than treated as a one-time fix.
