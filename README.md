...
A standalone Topic Planner Agent has also been implemented and real-validated: it automatically selects one video topic (evergreen/YouTube-popular, current/trending news via free Google News RSS, or both merged) instead of a human typing a topic before each run - deterministic freshness scoring, duplicate/history prevention (against both real production history and its own past selections), near-duplicate normalization, market/category preference, and cross-source confidence, combined with at most one bounded Gemini batch-ranking call and a deterministic fallback if that call fails. Target market/language/category are plain configuration, never hardcoded to any specific country or region. A real trending-mode validation (`python -m src.topic_planner_demo`) discovered 15 real current candidates and selected "The evolution of BRICS and its challenges today". Like the Upload Agent before its own integration milestone, the Topic Planner is standalone only for now - `src/topic_planner_demo.py` is a separate CLI, with no wiring into `src/workflows/pipeline_graph.py`/`src/pipeline_demo.py` yet. The next planned milestone is Autonomous Orchestration/Trigger (connecting topic selection → pipeline → publishing without a human running commands manually) - see `docs/PROGRESS.md`.
