...
A standalone YouTube Upload & Scheduling Agent has been implemented and real-validated (real OAuth authentication, real channel verification, a real private upload with its thumbnail applied, idempotent publishing records, and validated scheduling logic), and is also now reachable directly from the main pipeline above via an explicit opt-in `publishing` node added after Compliance: a normal pipeline run still never uploads anything, even on a genuine PASS, unless a caller explicitly supplies a `PublishingIntent` (`mode="private"`/`"scheduled"`, default `"disabled"`) and a `YouTubeClient`. Either path - standalone CLI (`src/youtube_upload_demo.py`) or the pipeline's own `publishing` node - reuses the identical `YouTubeUploadAgent`, OAuth handling, idempotency (`PublishingRecordStore`), and compliance gate: publishing proceeds only when `compliance_result.publish_decision == "PASS"`, and a `REVIEW` or `BLOCK` decision always refuses to upload, with no bypass. A bounded Compliance Remediation loop (REVIEW → targeted script correction → fresh re-evaluation, capped at 2 attempts) runs directly inside the main pipeline's Compliance stage. One controlled real private integrated publishing validation has succeeded end to end (real OAuth reuse, real upload, real thumbnail, persisted publishing record, idempotency verified, manually confirmed Private in YouTube Studio); real scheduled publishing remains validated only via dry-run/mocked tests, not a real scheduled upload yet. Automated cleanup/retention is not implemented yet.

A standalone Topic Planner Agent has also been implemented and real-validated: it automatically selects one video topic (evergreen/YouTube-popular, current/trending news via free Google News RSS, or both merged) instead of a human typing a topic before each run - deterministic freshness scoring, duplicate/history prevention (against both real production history and its own past selections), near-duplicate normalization, market/category preference, and cross-source confidence, combined with at most one bounded Gemini batch-ranking call and a deterministic fallback if that call fails. Target market/language/category are plain configuration, never hardcoded to any specific country or region. A real trending-mode validation (`python -m src.topic_planner_demo`) discovered 15 real current candidates and selected "The evolution of BRICS and its challenges today". `src/topic_planner_demo.py` and `src/pipeline_demo.py` remain separate manual CLIs (and `src/workflows/pipeline_graph.py` itself has no Topic Planner node), but a human is no longer required to bridge them - see Autonomous Orchestration below.

**Autonomous Orchestration** removes the need for a human to run `pipeline_demo.py` or type a topic at all. `AutonomousRunController` (`src/orchestration/`) is a coordination-only layer - it contains no Research/Script/Video/Compliance/YouTube logic of its own - that calls the existing `TopicPlannerAgent.plan_topic()` and then the existing `run_pipeline(...)` directly, unchanged, in one run: acquire a durable file-based run lock (never an in-memory flag; stale locks auto-recover; always released even on exception) → select a topic (or record `no_topic` safely) → run the full content pipeline → Compliance/Remediation → Publishing per an explicit `PublishingIntent` → persist a durable `AutonomousRunRecord` → release the lock. A minimal stdlib-only cron scheduler (`AutonomousScheduler`, no Celery/Redis/Kafka/APScheduler) can trigger this on a configured schedule (`AUTONOMOUS_SCHEDULE`/`AUTONOMOUS_TIMEZONE`, disabled by default), and the exact same controller is reachable manually via `python -m src.autonomous_runner --run-once` (or `--dry-run` for planning-only, no pipeline call) - no separate manual-vs-scheduled code path. Publishing defaults to `AUTONOMOUS_PUBLISHING_MODE=disabled`, so a fresh deployment never auto-publishes. **One controlled real autonomous end-to-end run has succeeded**: automatic topic selection ("Why does ice float on water?"), a real full pipeline run, a genuine Compliance PASS with 0 remediation attempts needed, and a real PRIVATE YouTube upload - to a newly authorized dedicated channel, "What's Behind It" (the uploader's OAuth authorization was migrated from the original personal/test channel via the existing, unmodified `--authenticate` flow; the same OAuth client credentials are reused across channels, only the token changes). The next planned milestone is production scheduling configuration and real scheduler-trigger validation, followed by deployment planning - see `docs/PROGRESS.md`.
